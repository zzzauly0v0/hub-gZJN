...
**按项目实际技术栈划分链路**（不是所有项目都有以下全部，按需增减）：
- 前端链路：UI 入口 → 请求封装(axios/fetch/SSE) → 事件处理 → 用户交互(点击/确认/表单)
- 后端入口链路：Controller/路由 → 鉴权 → 参数校验 → 分发/预处理
- 核心业务链路：业务 Service → 算法/规则/编排（AI 项目可能是 Agent/检索/工具；CRUD 项目可能是事务/状态机/校验）
- 数据链路：DB 访问层(ORM/MyBatis) → 缓存(Redis) → 外部存储(向量库/ES/对象存储)
- 离线/初始化链路：配置加载 / 数据同步 / 建索引 / 定时任务 / 训练（按项目有无此类决定是否探索）
...
- 识别**前置代码**：链路中需事先完成的离线/初始化逻辑（如建索引/数据同步/配置加载/缓存预热/训练/数据迁移等，按项目实际而定）
...
- 举例（按项目类型选 relevant 的）：
  - AI 项目：chunking 为什么按语义单元切而非固定 token；为什么用质心分类而非 SVM；LoRA 为什么只训注意力层
  - 后端项目：为什么用 Redis 缓存而非本地缓存；为什么消息队列异步而非同步；为什么乐观锁而非悲观锁
  - 前端项目：为什么用 SSE 而非 WebSocket；为什么状态管理用 Vuex/Pinia 而非 Redux；为什么虚拟列表而非全量渲染
...
| ③ 端到端流水线 | 分阶段表（阶段名按项目实际定，如"初始化→接入→处理→执行→沉淀"，AI 项目可能有"离线/分流"阶段，CRUD 项目可能是"鉴权→校验→事务→落库"，按需增减），每行含技术/代码/产物 |
...
- **前置逻辑是亮点**：用户最常困惑的是"链路中那段代码凭什么能跑"——主动识别并解释前置依赖。AI 项目如"检索前向量库是提前 chunking 的，chunking 做法是…"；后端项目如"接口能查到数据是因为启动时缓存预热了"；前端项目如"页面能拿到选中行是因为 store 在双击时注入了上下文"。
