...
作业:改用交叉熵实现一个多分类任务，五维随机向量最大的数字在哪维就属于哪一类。
"""
...
class TorchModel(nn.Module):
    ...
    # 前向计算 当输入真实标签，返回loss值；无真实标签，返回预测值
...
# 随机生成一个5维向量，五个维度中最大的值，认为是正样本，反之为负样本
def build_sample():
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            ...
            if np.argmax(y_p, axis=0) == np.argmax(y_t, axis=0):
                correct += 1  # 样本判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():  # 不计算梯度
        ...
        ndarray = np.zeros(result.shape)
        for i,res in enumerate(result):
            argmax_index = res.argmax().item()
            ndarray[i][argmax_index] = 1
    np_array = np.array(input_vec)
    for vec, res in zip(np_array, ndarray):
