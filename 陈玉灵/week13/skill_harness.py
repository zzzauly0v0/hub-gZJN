...
参数说明：
  ...
  --skill     指定 SKILL.md 文件路径，默认当前目录下 SKILL.md。
  --word      指定要生成的单词，跳过输入解析。
  --output    指定 HTML 输出路径，默认当前目录下 <word>.html。
...
class SkillHarness:
    ...
    def __init__(self, skill_file: Path):
        ...
        self.skill_root = skill_file.parent
        self.data_dir = self.skill_root / "data"
        self.script_path = self.skill_root / "scripts" / "make_flashcard.py"
...
def main() -> int:
    ...
    parser.add_argument("--skill", default="SKILL.md", help="技能定义文件路径。")
    parser.add_argument("--word", help="指定要生成的单词，跳过输入解析。")
    parser.add_argument("--output", help="HTML 输出路径，默认当前目录下 <word>.html。")
